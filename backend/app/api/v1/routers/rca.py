...
from app.schemas.workflows import RCACreate, IdempotencyResponse
...
router = APIRouter(prefix="/projects/{project_id}/quality-events/{event_id}/rca", tags=["RCA"])
...
async def submit_rca(
    ...
    result = await workflow_service.submit_rca(
        session=db,
        event=event,
        rca_data=payload.model_dump(by_alias=True),
